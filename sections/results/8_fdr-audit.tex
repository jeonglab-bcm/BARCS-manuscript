...
We audited this point using the deposited null-only and full-gene result
tables.  The published subset-matrix analysis
...
to equal model input.  The same limitation applies to the ComBat-corrected,
renormalized, and rounded Liang matrix used in this manuscript.  Both analyses
are labeled processed-count sensitivity checks, and neither is used to validate
a count likelihood.
...
The appropriate conclusion is therefore narrower than either
``CB\textsuperscript{2} is miscalibrated'' or ``BARCS solves false
discovery.''
