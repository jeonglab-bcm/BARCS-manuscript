% !TeX root = ../../main.tex
\subsection{An ordered-bin design trades validation yield per discovery for
sensitivity}
\label{sec:facs}
...
controls used to estimate that fold's scale.  Validation yield is the number
of validated regulators recovered divided by all discoveries; it is a
descriptive efficiency ratio, not an odds ratio.}
\label{tab:waterbear}
\small
...
yields of 44.9\% and 23.6\%, respectively.  The four-bin BARCS model also
recovered four regulators missed by its outer-bin fit, but required 15
additional discoveries to do so.  Its marginal validation yield was therefore
4/15 (26.7\%), and its overall yield was lower than the outer-bin fit
(44.9\% versus 52.9\%).  The intermediate bins increased sensitivity to the
validated set at the cost of precision per call; they did not uniformly
improve efficiency.
...
BARCS therefore offered the highest recovery-per-discovery ratio among the
four-bin models shown (44.9\%), although both outer-bin fits were more
efficient per call (52.9\% for BARCS and 60.0\% for MAGeCK test), and
Waterbear remained the more faithful generative model for correlated sorted
bins.  Negative controls diagnose the independence violation and calibrate one
operating point; they do not make the bin margins independent.
