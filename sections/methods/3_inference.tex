...
make the gene statistic anti-conservative.  That cost is measured rather than
assumed.  In the all-null grid of Table~\ref{tab:calibration-grid}, imposing a
within-gene guide correlation of 0.4 left guide-level type-I error at or below
0.048 while raising Stouffer gene-level error to 0.108--0.144 for three-guide
genes and 0.224--0.262 for five-guide genes; aggregation-matched split-control
scaling reduced the excess to 0.056--0.112 but did not remove it.  Every
gene-level rate reported in this paper uses this combiner and should be read
with that residual inflation in mind.  A production extension should
