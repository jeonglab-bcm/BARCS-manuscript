...
low--bulk--high analyses, column maxima for ranking, effect recovery, recall,
and F1 are highlighted.}
...
0.764$\pm$.089 & 0.086$\pm$.044 & 0.828$\pm$.037\\
\methodswatch{mageckorange} MAGeCK-MLE & Low--bulk--high &
0.964$\pm$.009 & 0.924$\pm$.018 &
...
\textcolor{chronosgreen}{\textbf{0.965$\pm$.006}} &
\textcolor{chronosgreen}{\textbf{0.932$\pm$.015}} &
0.924$\pm$.013 & 0.886$\pm$.021 & 0.232$\pm$.053 &
0.822$\pm$.034\\
\methodswatch{mageckpink} DESeq2 + Stouffer & Low--bulk--high &
0.963$\pm$.009 & 0.930$\pm$.016 & 0.921$\pm$.013 &
0.884$\pm$.021 & 0.240$\pm$.047 & 0.817$\pm$.031\\
\methodswatch{bagelgold} limma-voom + Stouffer & Low--bulk--high &
0.965$\pm$.005 & 0.932$\pm$.016 & 0.925$\pm$.012 &
\textcolor{bagelgold}{\textbf{0.890$\pm$.017}} &
0.218$\pm$.053 &
\textcolor{bagelgold}{\textbf{0.832$\pm$.036}}\\
\midrule
...
The result is a tradeoff, not a universal victory.  In the low--bulk--high
design, edgeR-QL and limma-voom have the highest global ranking metrics, and
MAGeCK-MLE has the highest effect correlation.  The three general count
models also have directional recall of 0.884--0.890, but their realized FDP is
0.218--0.240: more than twice the nominal 0.10 target after the common
Stouffer gene summary.  Their high recall is therefore not a calibrated win.
BARCS and MAGeCK-MLE have lower realized FDP, 0.086 and 0.048, respectively.
BARCS has higher directional recall than MAGeCK-MLE and similar F1: 0.828
versus 0.817 in the low--bulk--high design and 0.786 versus 0.706 in the
two-tail design.  Negative-control $p<0.05$ frequencies averaged 0.061 and
...
\caption{Four BARCS guide-to-gene statistics in one representative
four-replicate CRISPulator realization.  All four reuse the same fitted
guide-level beta-binomial coefficients.  (A) Truth-recovery metrics for the
low--bulk--high analysis.  (B) Realized FDP among gene FDR-0.10 calls; the
dashed line is the nominal target.  (C) Negative-control fraction with
$p<0.05$; the dashed line is the nominal rate.  The comprehensive comparison
uses five seeds in each of ten parameter settings.}
\label{fig:crispulator}
\end{figure}

\paragraph{Sensitivity to MOI, guide quality, library size, and replication.}
...
The same 50 runs compared four guide-to-gene rules without refitting the
guide-level model.  Mean average precision was 0.856 for BARCS-ST
(\texttt{BARCS-original}), 0.706 for BARCS-NORM, 0.779 for BARCS-RE
(\texttt{BARCS-partial}), and 0.840 for BARCS-RE-EB
(\texttt{BARCS-EB}).  Their directional recalls at gene FDR 0.10 were 0.638,
0.159, 0.390, and 0.463, respectively; mean realized FDPs were 0.085, 0.059,
0.052, and 0.013.  BARCS-NORM's negative-control $p<0.05$ rate was close to
nominal at 0.047, but estimating a separate spread from approximately five
guides left only four reference degrees of freedom and sharply reduced
threshold power.  The historical signed-score statistic therefore remains
the strongest default in these simulations, while the moderated
random-effects statistic provides the most conservative operating point.
...
\begin{figure}[!t]
\centering
\includegraphics[width=\textwidth]{figures/crispulator_facs_multimethod_replicates.pdf}
\caption{Four BARCS guide-to-gene statistics as biological screen replication
increases.  Points are five-seed means for the low--bulk--high design.
(A) Average precision, (B) directionally correct recall at gene FDR 0.10, and
(C) realized FDP; the dotted line is the nominal 0.10 target.  The identical
guide-level fit is reused for every method.  The one-replicate setting is a
diagnostic boundary rather than confirmatory inference.}
\label{fig:crispulator-barcs-replicates}
\end{figure}
...
but their apparent recall advantage is coupled to FDP near 0.20--0.25.  At six
...
\caption{Head-to-head comparison across all 50 low--bulk--high CRISPulator
runs.  Bars are means and error bars are standard errors.  (A) Average
