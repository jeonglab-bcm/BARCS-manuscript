...
The realized-FDP column of Table~\ref{tab:crispulator} is a single point, so
it cannot distinguish a method that controls the false-discovery rate from one
that happens to land near 0.10.  We therefore repeated the calibration check
at eight gene-FDR cutoffs from 0.001 to 0.50 in every run.  The quantity of
interest is the ratio of realized FDP to the cutoff that was requested: a
value of one is exact calibration, below one is conservative, and above one
means the method returns more false discoveries than it advertised.

\begin{table}[ht]
\centering
\caption{Realized FDP divided by the requested gene-FDR cutoff, MOI 0.20,
means over three seeds on the common gene set.  One is exact calibration and
values above one are violations.  The final column counts the
cutoff-by-seed cells, of $8\times3=24$, in which realized FDP did not exceed
the cutoff.}
...
\toprule
Method & 0.001 & 0.005 & 0.01 & 0.05 & 0.10 & 0.20 & 0.30 & 0.50 &
Cells held\\
\midrule
\methodswatch{barcsblue} BARCS-ST &
0.59 & 0.73 & 0.59 & 0.61 & 0.65 & 0.67 & 0.66 & 0.72 & 22/24\\
\methodswatch{mageckorange} BARCS-MOD &
0.40 & 0.63 & 0.68 & 0.62 & 0.66 & 0.68 & 0.69 & 0.73 & 22/24\\
\methodswatch{naivegray} MAGeCK-MLE &
0.00 & 0.13 & 0.06 & 0.08 & 0.06 & 0.10 & 0.14 & 0.20 &
\textcolor{black}{\textbf{24/24}}\\
\methodswatch{chronosgreen} edgeR-QL + Stouffer &
9.96 & 5.64 & 4.09 & 2.52 & 2.05 & 1.59 & 1.37 & 1.11 & 0/24\\
\methodswatch{mageckpink} DESeq2 + Stouffer &
17.15 & 7.77 & 5.86 & 3.19 & 2.39 & 1.77 & 1.50 & 1.15 & 0/24\\
\methodswatch{bagelgold} limma-voom + Stouffer &
10.92 & 5.47 & 4.76 & 2.73 & 2.14 & 1.66 & 1.41 & 1.12 & 0/24\\
\bottomrule
\end{tabular}
\end{table}

The direction of miscalibration is a property of the method rather than of the
0.10 choice.  edgeR-QL, DESeq2, and limma-voom exceed the requested rate at
every one of the eight cutoffs and in all 24 cutoff-by-seed cells, and the
excess is largest exactly where a user is being most careful: at a requested
0.001 they return 10 to 17 times that rate.  Their apparent convergence toward
one at the loosest cutoffs is not improved calibration but the ceiling effect
of a ratio whose numerator cannot exceed the fraction of inactive genes.
MAGeCK-MLE holds in all 24 cells and never exceeds 0.21 of the requested rate.
Both BARCS statistics sit between 0.40 and 0.73 of the requested rate at every
cutoff, the low end being BARCS-MOD at the strictest 0.001, which is the
behaviour a screen can plan around.

The BARCS exceptions are worth naming rather than rounding away.  Both
statistics miss in 2 of 24 cells, and every miss comes from one seed
(20250725) at the 0.001 and 0.005 cutoffs, where the excess is a single false
discovery: at a requested 0.001 with 840 calls, BARCS-MOD returns FDP 0.0012
against an allowance of 0.84 genes.  These are Monte Carlo granularity at the
strictest cutoffs, not a systematic failure, and they are the reason the
calibration claim in this paper is stated as a screen-average rather than a
finite-sample guarantee.  At MOI 0.30 the same pattern holds, with the one
additional miss for BARCS-MOD at the 0.01 cutoff of the same seed.
...
\emph{The ordering of methods by F1 is not robust to the cutoff.}  At a
requested 0.001 the ranking is DESeq2 0.795, limma-voom 0.763, edgeR-QL 0.757,
then BARCS-MOD 0.607, BARCS-ST 0.456, and MAGeCK-MLE 0.306; at
0.01 DESeq2 still leads at 0.847 against 0.748 for BARCS-MOD; at 0.10
BARCS-MOD leads at 0.847; and by 0.50 MAGeCK-MLE leads at 0.853.  BARCS-MOD
holds the top F1 only at cutoffs of 0.10 and above.  A reader who preferred a
stricter screen would reach the opposite conclusion from
Table~\ref{tab:crispulator}.

The reason is that a fixed requested cutoff does not put these methods at the
same place.  Table~\ref{tab:crispulator-fdr-robust} showed the count models
running at 4 to 17 times their requested rate at strict cutoffs, so at a
requested 0.001 they are in fact operating near a realized 0.01--0.02 and
calling far more genes.  Comparing F1 there compares a liberal operating point
with a conservative one.

Matching on realized FDP instead removes the confound, and the result is that
F1 does not separate these methods at all:
...
across the five methods other than BARCS-ST.}
...
\midrule
\methodswatch{barcsblue} BARCS-ST &
0.673 & 0.722 & 0.795 & 0.819 & 0.816\\
\methodswatch{mageckorange} BARCS-MOD &
0.759 & 0.792 & 0.838 & 0.847 & 0.828\\
\methodswatch{naivegray} MAGeCK-MLE &
0.739 & 0.792 & 0.830 & 0.853 & ---\\
\methodswatch{chronosgreen} edgeR-QL + Stouffer &
0.764 & 0.787 & 0.835 & 0.843 & 0.826\\
\methodswatch{mageckpink} DESeq2 + Stouffer &
--- & 0.800 & 0.842 & 0.844 & 0.826\\
\methodswatch{bagelgold} limma-voom + Stouffer &
0.764 & 0.794 & 0.834 & 0.840 & 0.826\\
\midrule
...
At every matched error rate the five modern methods lie within 0.002 to 0.025
of one another, and which one is nominally highest changes from one row to the
next.  We therefore do not claim that guide-dispersion moderation recovers more
genes than the alternatives: at a common realized error rate it does not, and
neither does anything else here.  Peak F1 tells the same story, spanning only
0.827 to 0.853 across methods with the maximum belonging to MAGeCK-MLE.

Two claims do survive the scan.  First, the improvement of BARCS-MOD over
BARCS-ST is robust to the cutoff: it is larger at every matched realized FDP,
by 0.086, 0.070, 0.042, 0.028, and 0.012 at the five levels, so moderation is
a genuine gain within the BARCS family rather than a threshold artifact.
Second, and this is the substantive result, BARCS-MOD is the method whose
requested cutoff lands where the F1 curve is already flat and high while the
realized error rate is still below what was asked for.  F1 does not
discriminate between these methods; what discriminates them is whether the
