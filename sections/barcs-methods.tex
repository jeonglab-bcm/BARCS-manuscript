...
result.  The comparators are CRISPR-specific gene callers rather than general
...
DESeq2 therefore appears as a preprocessing step and is not itself scored.
...
Growth was logistic.  The alternative exponential model is unbounded, and over
five days it let a single guide reach a fifth of the library and drove a
175-fold spread in library size, which is not a pooled screen.  Initial
...
within a gene, and simCRISPR gives every sgRNA an independent interaction, so
grouping guides into synthetic genes would manufacture the structure it
exploits.
