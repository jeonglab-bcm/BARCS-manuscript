...
effect model.  In the head-to-head benchmarks, this Stouffer--median procedure
is used only for BARCS (and for the deliberately naive binomial comparator).
MAGeCK, Chronos, BAGEL2, Waterbear, and MAUDE retain their native or deposited
gene-level summaries.  The exploratory DeWeirdt false-discovery audit used
Fisher aggregation as stated in its Results description and is not pooled
with these primary benchmark statistics.
...
the baseline $(\mathrm{MOI},q,G)=(0.25,0.90,400)$, where $q$ is the
high-quality-guide fraction and $G$ is the number of genes.  The evaluated
levels were $\mathrm{MOI}\in\{0.10,0.25,0.40\}$,
$q\in\{0.60,0.75,0.90\}$, and $G\in\{100,400,1000\}$.  After removing
duplicate baseline settings, this yielded seven scenarios; each used the same
five prespecified seeds, for 35 simulations.  One parameter changed at a time,
so its main sensitivity remained interpretable.  For each seed and scenario,
we paired BARCS and MAGeCK-MLE results from the low--bulk--high design and
reported differences as BARCS minus MAGeCK-MLE.  The executable benchmark
also supports the complete $3\times3\times3$ factorial when interaction
...
\]
while bulk received $d_{\mathrm{bulk}}=0$.  Both BARCS and MAGeCK-MLE fitted
this score with replicate indicators.  Tail-only fits removed bulk without
changing the low and high counts.  BARCS guide tests were scaled with the
simulated negative-control guides before directional Stouffer aggregation;
MAGeCK-MLE used its native gene-level Wald result.
