...
coefficient and model-based standard error only as a signed precision score,
\begin{equation}
  u_{gj}
  =
  \frac{\widehat{\beta}_{gj}}
       {\operatorname{SE}(\widehat{\beta}_{gj})},
  \qquad
  S_g
  =
...
\end{equation}
Thus several concordant guide $t$ scores reinforce one another, whereas
discordant guide directions cancel.  No guide $p$-value is used.

Because $S_g$ does not have a trustworthy sample-replicate reference
distribution at $R=1$, it is calibrated across genes.  Let $c_0$ be the
all-gene median and
\begin{equation}
  s_{\mathrm{MAD}}
  =
  \frac{\operatorname{median}_g|S_g-c_0|}
       {\Phi^{-1}(0.75)}.
  \label{eq:gcmad}
\end{equation}
...
\begin{equation}
  Z_g^{\mathrm{GC}}
  =
  \frac{S_g-c_C}{\max(1,s_{\mathrm{MAD}},s_C)},
  \qquad
  p_g^{\mathrm{GC}}
  =
  2\Phi(-|Z_g^{\mathrm{GC}}|),
...
guide scores are required per gene.  The all-gene MAD assumes that fewer than
half of tested genes are active.
