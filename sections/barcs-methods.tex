...
$R\in\{3,4,6\}$.  After removing duplicate baseline settings, this yielded
nine scenarios; each used the same five prespecified seeds, for 45
simulations.  One parameter changed at a time, so its main sensitivity
remained interpretable.  For each seed and scenario, we paired BARCS and
MAGeCK-MLE results from the low--bulk--high design and reported differences as
BARCS minus MAGeCK-MLE.  The executable benchmark also supports the complete
$3^4$ factorial (81 scenarios and 405 five-seed simulations) when interaction
effects are the target.
