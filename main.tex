...
and prespecified linear contrasts.  We prove that the original
CB\textsuperscript{2} statistic is exactly a binary-design generalized
least-squares contrast and that, under local alternatives, the logit-scale
regression statistic is first-order equivalent to it.  Guide counts remain the response; sample
...
Four complementary benchmarks define what this generalization gains and where
...
CB\textsuperscript{2}-Reg is therefore best viewed as the regression form of
CB\textsuperscript{2}: a fast, transparent general-purpose method for
...
default logit fit reproduces the original finite-sample statistic.  The exact
and asymptotic relationships are proved below.
...
\subsection{Exact embedding and local-equivalence proof}

We now state exactly in what sense CB\textsuperscript{2}-Reg generalizes
CB\textsuperscript{2}.  For the original test, let
\[
...
\begin{proposition}[Exact binary-design representation]
\label{prop:exact}
...
\begin{corollary}[Exact recovery of legacy inference]
If the slope statistic in Proposition~\ref{prop:exact} is referred to
...
Thus the original two-group test is an exact special case of
design-matrix contrast inference on the identity-link, group-summary scale.
\end{corollary}
...
$n_A,n_B\rightarrow\infty$, both its residual degrees of freedom and
Equation~\eqref{eq:legacydf} diverge, so their reference distributions also
converge to the same standard normal limit.
\end{proposition}
...
statistic is \emph{exactly nested} in design-matrix contrast algebra, while
the implemented logit statistic is \emph{locally asymptotically equivalent}
in the unadjusted binary design.  In finite samples,
...
Proposition~\ref{prop:exact} to machine precision and illustrates the
...
CB\textsuperscript{2}-Reg turns a successful two-group idea into a regression
framework without discarding what made the original method useful.  Counts
...
and nonlinear time or dose effects.
