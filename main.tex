...
and prespecified linear contrasts.  Guide counts remain the response; sample
...
depth-aware, extra-binomial variance and changes the mean model from two group
means to a design matrix.
...
Mean parameter & Group-specific guide proportions &
Logit-linear guide proportion\\
Effect & Difference or fold change between groups &
Conditional log-odds coefficient or linear contrast\\
Variance principle & \multicolumn{2}{c}{Binomial sampling plus
between-library beta-binomial heterogeneity}\\
Inference & Two-group weighted statistic &
Coefficient $t$ statistic with $m-q$ sample-level degrees of freedom\\
...
The appropriate bridge was described by \citet{baggerly2004overdispersed} for
...
\subsection{Connection with the original two-group test}

If $d_i$ is replaced by a zero--one group indicator, Equation~\eqref{eq:mean}
becomes a two-group beta-binomial regression.  Its coefficient tests a
log-odds contrast rather than CB\textsuperscript{2}'s raw difference in
proportions, so the two statistics are related but not algebraically identical.
For rare guides and modest effects, the logit is approximately the log
proportion and the conclusions should usually be similar.  Regression adds the
ability to use all dose levels together and to adjust for batch or other
covariates.
